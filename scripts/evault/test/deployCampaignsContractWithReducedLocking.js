...
async function main() {
  ...
  let freezingPeriodInSeconds = process.env.FREEZING_PERIOD_IN_SECONDS;
  console.log(`Lock period in seconds: ${freezingPeriodInSeconds}`);

  if (!freezingPeriodInSeconds)
    throw "Configure reduced lock period as FREEZING_PERIOD_IN_SECONDS environment variable!";

  // deploy campaigns as sce
  let factory = await ethers.getContractFactory("TestEVaultCampaignsWithReducedLocking");
  const campaigns = await factory.connect(sce).deploy(nftAddress, freezingPeriodInSeconds);
  await campaigns.deployed();
  console.log(`Campaigns contract: ${campaigns.address}`);
  const c = factory.attach(campaigns.address);
  console.log(`Freezing period queried from contract: ${await c.getFreezingPeriodInSeconds()} sec`);
